...
const DEFAULT_CLIMATE_DONUT = {
  labels: ["Evenement transport", "Deelnemer transport", "Voedsel", "Accommodatie (energie)", "Kleding"],
  datasets: [
    {
      data: [42, 51, 5, 2, 0],
      backgroundColor: ["#c72b3b", "#003f7f", "#f7b733", "#2f9e44", "#9e005d"],
      borderWidth: 0,
    },
  ],
};

const solarLineOptions = {
  responsive: true,
  maintainAspectRatio: false,
  plugins: {
  ...
  scales: {
    y: {
      ...
      ticks: { callback: (v) => v + "%", stepSize: 2, max: 20 },
    },
    x: { grid: { display: false } },
  },
};

const solarBarOptions = {
  responsive: true,
  maintainAspectRatio: false,
  plugins: {
...
const SolarEnergyDashboard = () => {
  ...
  useEffect(() => {
    const durations = [10000, 20000, 20000];
    const t = setTimeout(() => setScreen((p) => (p + 1) % 3), durations[screen]);
  ...
  const solarLineData = liveData?.solarLine || DEFAULT_SOLAR_LINE;
  const solarBarData = liveData?.solarBar || DEFAULT_SOLAR_BAR;
  const climateData = liveData?.climateImpact || DEFAULT_CLIMATE_DONUT;
      ...
      >
        <div className="w-full h-full max-w-[1920px] mx-auto flex flex-col gap-4 py-4 px-4">
          ...
          <div className="px-1 py-1 shrink-0">
            <img src="/assets/logo.png" alt="logo" className="w-[58px] h-[58px] rounded-full object-contain shadow-sm" />
          </div>
                ...
                <div className="flex-1 min-h-0">
                  <Doughnut
                    data={{
                      ...
                      datasets: [{ data: [68, 32], backgroundColor: ["#766fd0", "#35bfd0"], borderWidth: 0 }],
                    }}
                    options={{
                      maintainAspectRatio: false,
          ...
          {/* screen 2 – 3 solar graphs */}
          {screen === 2 && (
            <div className="grid grid-cols-3 gap-4 flex-1 min-h-0">
              <div className="bg-white rounded-[20px] p-5 flex flex-col min-h-0 transition-all hover:shadow-md">
                <p className="text-[15px] font-semibold text-gray-800 mb-1">
                  Opbrengst Zonnepanelen per maand
                </p>
                <p className="text-[12px] text-gray-500 mb-3">
                  (als % van de totale opbrengst per jaar 2015 tm 2022)
                </p>
                <div className="flex-1 min-h-0">
                  <Line data={solarLineData} options={solarLineOptions} />
                </div>
              </div>
              <div className="bg-white rounded-[20px] p-5 flex flex-col min-h-0 transition-all hover:shadow-md">
                <p className="text-[15px] font-semibold text-gray-800 mb-1">
                ...
                <p className="text-[12px] text-gray-500 mb-3">
                  (als % van de totale jaaropbrengst) · 2015 – 2022
                </p>
                <div className="flex-1 min-h-0">
              ...
              <div className="bg-white rounded-[20px] p-5 flex flex-col min-h-0 transition-all hover:shadow-md">
                <p className="text-[15px] font-semibold text-gray-800 mb-1">Impactmeting Climate Classic 2021</p>
                <p className="text-[12px] text-gray-500 mb-3">
                  Totale footprint 43.089 kg CO₂ eq.
                </p>
                <div className="flex-1 min-h-0 flex gap-4 items-center">
                  <div className="w-[50%] h-full">
                    <Doughnut data={climateData} options={climateDonutOptions} />
                  </div>
                  <div className="flex-1 text-[12px] space-y-2">
                    {climateData.labels.map((label, idx) => (
                      <div key={label} className="flex items-center gap-2">
                        <span
                          className="w-3 h-3 rounded-sm"
                          style={{ backgroundColor: climateData.datasets[0].backgroundColor[idx] }}
                        />
                        <span className="text-gray-700">{label}</span>
                        <span className="text-gray-500 ml-auto">
                          {climateData.datasets[0].data[idx]}%
                        </span>
                      </div>
                    ))}
                    <p className="text-[11px] text-gray-400 pt-2">
                      = energieverbruik 11,3 huishoudens voor 1 jaar
                    </p>
                  </div>
                </div>
              </div>
