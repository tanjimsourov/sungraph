// Page: ClimateClassicModernPage
// Description:
//   Presents a climate impact measurement example.  Shows CO₂ footprint with and
//   without transport using static donut charts and statistics.  Includes Dutch
//   and English translations.  The UI layout remains unchanged.
...
const translations = {
  nl: {
    ...
    leftCard: {
      ...
      footer: 'Berekeningen door Copper8 op basis van LCA (ReCiPe methodiek).',
    },
    centerCard: {
  ...
  en: {
    ...
    leftCard: {
      ...
      footer: 'Calculations by Copper8 based on the LCA (ReCiPe method).',
    },
    centerCard: {
...
export default function ClimateClassicModernPage() {
  ...
  return (
    <div
      ...
      style={{
        background: 'radial-gradient(circle at top, #f4f7fb 0%, #e3edf5 45%, #d7e3ee 100%)',
      }}
    >
      ...
      <header className="relative z-10 w-full px-8 pt-6 flex items-start justify-between gap-6">
        <div>
          <h1 className="text-[34px] font-bold text-[#062f53] leading-tight drop-shadow-sm">{t.title}</h1>
        ...
        <div className="bg-white/90 backdrop-blur-md border border-white/70 rounded-2xl px-6 py-4 shadow-[0_20px_45px_rgba(15,23,42,0.08)] min-w-[290px]">
          <p className="text-sm text-[#c93334] font-semibold">{t.totalFootprint}</p>
          <p className="text-[30px] font-bold text-[#c93334] leading-none mt-1 tracking-tight">{t.totalFootprintEq}</p>
          <div className="flex items-center gap-2 mt-3">
            <div className="flex gap-[3px]">
              {Array.from({ length: 8 }).map((_, idx) => (
                <svg key={idx} viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="#c93334" strokeWidth="1.5">
                  <path d="M4 11.5 12 5l8 6.5" />
                  <path d="M7 11V19h4v-4h2.5V19H18v-8" />
                </svg>
              ))}
            </div>
            <p className="text-[10.5px] text-[#c93334] italic">{t.householdsEq}</p>
          </div>
        </div>
      </header>
      ...
      <div className="relative z-10 w-full px-8 mt-4">
        <div className="w-full bg-white/80 backdrop-blur-md border border-white/70 rounded-2xl px-6 py-4 flex flex-wrap items-center justify-between gap-4 shadow-sm">
          <p className="text-sm text-slate-600 max-w-[55rem] leading-relaxed">{t.info.description}</p>
          <div className="flex gap-2">
            <span className="px-3 py-1 rounded-full bg-[#e3f1ff] text-[#062f53] text-xs font-semibold">{t.info.climateChip}</span>
            <span className="px-3 py-1 rounded-full bg-[#ffe1e1] text-[#c93334] text-xs font-semibold">{t.info.transportChip}</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 w-full px-8 mt-3 flex gap-3 flex-wrap">
        <div className="px-3 py-2 bg-white/40 rounded-xl text-xs text-slate-500 border border-white/50">
          {t.kpis.largestShare}
          <span className="font-semibold text-[#c93334]">{t.kpis.largestValue}</span>
        </div>
        <div className="px-3 py-2 bg-white/40 rounded-xl text-xs text-slate-500 border border-white/50">
          {t.kpis.withoutTransport}
          <span className="font-semibold text-[#062f53]">{t.kpis.withoutValue}</span>
        </div>
        <div className="px-3 py-2 bg-white/40 rounded-xl text-xs text-slate-500 border border-white/50">
          {t.kpis.method}
      ...
      <main className="relative z-10 flex-1 w-full px-8 pb-6 pt-4 overflow-hidden">
        <div className="w-full h-full grid grid-cols-1 xl:grid-cols-[1.05fr_0.8fr_1.05fr] gap-5 h-full">
          {/* Left card */}
          <div className="bg-white/90 backdrop-blur-md rounded-3xl border border-white/70 shadow-[0_16px_40px_rgba(15,23,42,0.04)] p-6 flex flex-col">
            <div className="mb-3">
              <p className="text-[15px] font-semibold text-[#062f53]">{t.leftCard.title}</p>
              <p className="text-[11px] text-slate-400">{t.leftCard.subtitle}</p>
            </div>
            <div className="flex items-center gap-6 flex-1">
              ...
              <div className="relative w-[230px] h-[230px]">
                <div
                  ...
                  style={{
                    background: 'conic-gradient(#c93334 0% 51%, #f5a9b0 51% 93%, #dfe5ef 93% 98%, #7d93d1 98% 100%)',
                  }}
                />
                <div className="absolute inset-9 bg-white rounded-full" />
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <p className="text-[11px] text-slate-400">totaal</p>
                  <p className="text-[13px] font-semibold text-[#062f53]">incl. tr.</p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 text-[13px]">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700">{t.leftCard.legend.participantTransport}</p>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700">{t.leftCard.legend.eventTransport}</p>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700">{t.leftCard.legend.food}</p>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700">{t.leftCard.legend.accommodation}</p>
                    <p className="text-[11px] text-slate-500">2%</p>
                  </div>
                </div>
              </div>
            </div>
            <p className="text-[11px] text-slate-400 mt-6">{t.leftCard.footer}</p>
          </div>
          ...
          <div className="bg-white/60 backdrop-blur-md rounded-3xl border border-white/50 shadow-[0_16px_40px_rgba(15,23,42,0.02)] p-6 flex flex-col items-center justify-center gap-4">
            <p className="text-[13px] text-slate-500">{t.centerCard.title}</p>
            <svg
              viewBox="0 0 500 200"
            ...
            <p className="text-[11px] text-slate-400 text-center max-w-[190px]">{t.centerCard.note}</p>
          </div>

          {/* Right card */}
          <div className="bg-white/90 backdrop-blur-md rounded-3xl border border-white/70 shadow-[0_16px_40px_rgba(15,23,42,0.04)] p-6 flex flex-col">
            <div className="mb-3">
              <p className="text-[15px] font-semibold text-[#062f53] text-right xl:text-left">{t.rightCard.title}</p>
              <p className="text-[11px] text-slate-400 text-right xl:text-left">{t.rightCard.subtitle}</p>
            </div>
            <div className="flex items-center gap-6 flex-1 justify-end xl:justify-start">
              ...
              <div className="relative w-[230px] h-[230px]">
                <div
                  className="w-full h-full rounded-full"
                  style={{ background: 'conic-gradient(#062f53 0% 73%, #3c6ca8 73% 97%, #d5e0ee 97% 100%)' }}
                />
                <div className="absolute inset-9 bg-white rounded-full" />
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                  <p className="text-[11px] text-slate-400">totaal</p>
                  <p className="text-[13px] font-semibold text-[#062f53]">excl. tr.</p>
                </div>
              </div>
              {/* Legend */}
              <div className="flex flex-col gap-4 text-[13px]">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700">{t.rightCard.legend.food}</p>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700">{t.rightCard.legend.accommodation}</p>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700">{t.rightCard.legend.clothing}</p>
                    <p className="text-[11px] text-slate-500">3%</p>
                  </div>
                </div>
              </div>
            </div>
            <p className="text-[11px] text-slate-400 mt-6 text-right">{t.rightCard.footer}</p>
          </div>
        </div>
      </main>
    </div>
